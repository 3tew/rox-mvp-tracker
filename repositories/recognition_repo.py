    ...
    for boss_name in config.BOSS_NAMES:
        ...
        if (boss_name in text) and (config.CURRENT_TIME - config.NOTICE_TIME) > 30:
            ...
            if 'spawn' in text:
                ...
                elif boss_name in ['amon ra', 'amon']:
                    webhook_repo.send_message_webhook(
                ...
                elif boss_name in ['dark priest']:
                    webhook_repo.send_message_webhook(
                        'refreshing', {"boss_data": config.BOSS_DATAS['darkpriest'], "is_abyss": isAbyss})
                elif boss_name in ['vagabond wolf', 'vagabond']:
                    webhook_repo.send_message_webhook(
                        'refreshing', {"boss_data": config.BOSS_DATAS['vagabondwolf'], "is_abyss": isAbyss})
